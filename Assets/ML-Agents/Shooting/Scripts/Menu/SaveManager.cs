...
public class SaveManager
{
    ...
    public static List<SaveInit> saveInits;
    ...
    private static SaveManager Instance
    {
        get
        {
            if(_instance == null)
            {
                _instance = new SaveManager(SaveData.Saves, SaveData.Path);
            }
            return _instance;
        }
    }
    ...
    {
        Instance.ReadFile(Instance._path);
        int size;
        if (typeof(T) == typeof(string))
            size = (Instance._container.GetLength(name) + 7) / 8;
        else
            size = DataSize<T>();
